...
void playMusic(const char * filename, int volume, int loop)
{
    Sound * global;
    Sound * newSound;
    uint8_t music;
    ...
    newSound = createSound(filename, loop, volume);
	newSound->is_music = 1;
    ...
    while(global != NULL)
    {
        ...
        if(global->is_music == 1 && global->fade == 0)
        {
            if(music)
            {
                global->length = 0;
        ...
        else if(global->is_music == 1 && global->fade == 1)
        {
            music = 1;
        }
